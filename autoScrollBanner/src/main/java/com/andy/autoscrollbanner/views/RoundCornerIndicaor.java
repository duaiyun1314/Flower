...
public class RoundCornerIndicaor extends View implements ViewPager.OnPageChangeListener {
    ...
    public RoundCornerIndicaor(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        int colorPrimary = array.getColor(0, context.getResources().getColor(R.color.colorPrimary));
        ...
        selectColor = ta.getColor(R.styleable.RoundCornerIndicaor_rci_selectColor, colorPrimary);
    ...
    public void setViewPager(ViewPager vp) {
        if (isValid(vp)) {
            this.vp = vp;
            final SwitchAdapter adapter = (SwitchAdapter) vp.getAdapter();
    ...
    private void drawSelect(Canvas canvas, int verticalOffset, int horizontalOffset) {
        ...
        int left = horizontalOffset + (indicatorWidth + indicatorGap) * currentItem + delta;
        if (left <= horizontalOffset + (count - 1) * (indicatorWidth + indicatorGap)) {
